...
const Sidebar = () => {
  ...
  return (
    <aside className="w-60 bg-[#030b18] border-r border-slate-800/60 flex flex-col h-full shrink-0">
      {/* Logo Section */}
      <div className="sticky top-0 z-20 bg-[#030b18]">
        <div className="flex items-center gap-3 p-4 border-b border-slate-800/60">
          <div className="w-8 h-8 bg-linear-to-br from-orange-500 to-amber-600 rounded-lg flex items-center justify-center shadow-md">
            ⚡
          </div>
            ...
            <NavLink
              ...
              {({ isActive }) => (
                <>
                  ...
                  {isActive && (
                    <span className="absolute right-3 w-1.5 h-1.5 bg-orange-500 rounded-full shadow-[0_0_8px_#f97316]" />
                  )}
                </>
              )}
            </NavLink>
      ...
      <div className="p-3 border-t border-slate-800/60">
        <div className="bg-[#0f172a] border border-slate-800 rounded-xl p-3 flex items-center gap-3">
          <Avatar className="w-9 h-9 border border-orange-500/30 bg-orange-500/10">
            <AvatarFallback className="text-orange-400 font-semibold text-xs">
